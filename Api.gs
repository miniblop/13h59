...
function _nomPropre(v) {
  return String(v == null  ''  v)
    .normalize('NFC')
     Caractu00E8res de formatage strictement invisibles u00E0 l'u00E9cran 
       00AD trait d'union conditionnel u00B7 180E su00E9parateur mongol
       200B-200F zero-width + marques gauchedroite u00B7 202A-202E encadrement bidi
       2060-2064 word joiner & invisibles mathu00E9matiques u00B7 2066-2069 isolats bidi
       FE00-FE0F su00E9lecteurs de variante u00B7 FEFF BOM
    .replace([u00ADu180Eu200B-u200Fu202A-u202Eu2060-u2064u2066-u2069uFE00-uFE0FuFEFF]g, '')
     s couvre du00E9ju00E0 l'espace insu00E9cable (00A0), les espaces typographiques
     (2000-200A), l'espace idu00E9ographique (3000) et les sauts de ligne.
    .replace(s+g, ' ')
    .trim();
}
...
function _caisseSave(body) {
 ...
 Variante GET (JSONP) — filet de secours si un jour on a un souci CORS.
   Appel  ...execaction=data&password=xxx&callback=cb  
function doGet(e) {
  const p = e.parameter  {};
 ...
 Diagnostic  dit quelle version est DÉPLOYÉE et quels mots de passe sont définis.
   Ne renvoie AUCUNE valeur de mot de passe, uniquement vraifaux.
   Ouvre ...execaction=ping dans le navigateur pour le lire. 
function _ping() {
  const props = PropertiesService.getScriptProperties();
